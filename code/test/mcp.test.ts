...
test("an MCP agent resolves and invokes an Aleph node, getting a verified receipt", async () => {
  ...
  // spawn the MCP server as a subprocess and connect a client to it
  const serverPath = fileURLToPath(new URL("../src/agent/mcp-server.ts", import.meta.url));
  ...
  try {
    ...
    const out = JSON.parse((acted.content as Array<{ text: string }>)[0].text);
