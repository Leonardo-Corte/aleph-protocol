...
import { SettlementRail } from "../src/settle/rail.ts";
import { createAttestation, verifyAttestation, computeTrust } from "../src/trust/attest.ts";
import { createNode } from "../src/node/node.ts";
import { createRegistry } from "../src/registry/registry.ts";
import { invoke, attest, fetchReputation, resolveRanked } from "../src/agent/client.ts";
...
test("computeTrust ignores unbacked attestations and weights by settled value", () => {
  ...
  const fake = { ...real[0], settlement: undefined as never }; // zero weight
...
test("end-to-end loop: pay -> receipt -> attest -> reputation -> ranked discovery", async () => {
  ...
  assert.equal(ranked[0].did, goodId.did);
  assert.ok(ranked[0].trust > ranked[1].trust);
